...
Define $VB = \{vb : v \in V, b \in B \}$.
If $B$ is an ideal, then $VB$ is a submodule of $V$.
...
Then the minimal polynomial of $r$ is $t^2 - r^2 \in {F_q^*}^2[t]$.
...
\]
and $k \subseteq (R + \frakr)/\frakr = \Rmodm$. Thus $k = \Rmodm$.
Consider two cases:
...
Thus by Wedderburn's theorem, $k \langle f_1, e_1 \rangle$ is a field, implying that $f$ and $e$ commute.
...
Let $a = h(u_1, u_1) \in \Rstar$ and $b = h(u_2, u_2) \in \Rstar$.
Let $c = b/a$.
...
0 \ne t_1^2 + ct_2^2.
\]
Because the choices of $t_1$ and $t_2$ were arbitrary, $-c$ cannot be a square in $\Rstar$.
...
whose corresponding norm map $J: S \to R$ is given by $s \mapsto s \hat{s}$, or $t_1 + t_2 \delta \mapsto t_1^2 + c t_2^2$.
Note that for $s \in S^\times$, $J(s) \in \Rstar$.
Then for an element $z \in S$, if $J(z) \in \frakm$, $z \not\in S^\times$.

We claim that $S$ is local with maximal ideal $S\frakm$.
To show this, we want to show that every non-invertible element of $S$ is contained in $S\frakm$.
To do this, we'll show that for $s \in S$, $J(s) \not\in \frakm$ if $s \not\in S \frakm$.
...
This implies that both $t_1, t_2 \in \frakm$, a contradiction.
...
\[
-c = (t_2^{-1})^2(t_1^2 - f) = (t_2^{-1})^2 t_1^2(1 - (t_1^{-1})^2f).
\]
Local rings have the property that if $m$ is a nonunit, $1 - m$ must be a unit.
Then $1 - (t_1^{-1})^2 f \in \Rstar$.
Using the assumption that the squaring map of $1 + \frakm$ is an epimorphism, we see that $1 - (t_1^{-1})^2 f \in \Rstarsq$,
implying that $-c \in \Rstarsq$, a contradiction.
Thus $t_1 + t_2 \delta \in S^\times$.
\end{description}
Therefore $S\frakm$ is an ideal of $S$, containing every non-unit, implying that it is maximal and that $S$ is local.

Because $S$ is commutative, $S/S\frakm$ is a field. The embedding $\Rmodm \to S/S\frakm$ allows us to view $S/S\frakm$ as a vector space over $\Rmodm$,
with $\{1 + S\frakm, \delta + S \frakm \}$ as a basis. Thus $S/S\frakm$ is a quadratic extension of $\Rmodm$.
The involution of $S$ induces the $\Rmodm$-automorphism of $S/S\frakm$ of order 2 and the norm map $J$ induces the norm map $(S/S\frakm)^\times \to (R/\frakm)^\times$.
...
Then by the surjectivity of the norm map $Q$, which induces $J$, there is $s \in S$ and $f \in \frakm$ such that
\[
J(s) = e + f = e(1 + e^{-1}f).
\]
Since $1 + e^{-1} f \in \Rstarsq$ (by surjectivity of squaring map of $1 + \frakm$), it follows that $e$ is in the image of $J$, as claimed.
Thus $J$ is surjective.
...
Recall that $\varep$ is an arbitrary fixed element of $\Rstar \backslash \Rstarsq$.
% Theorem 3.5
...
\begin{description}
\item[Case 1:] $h(u_1, u_1) \in \Rstarsq$.
Then there exists $v$ such that $h(v,v) = 1$, as shown.
\item[Case 2:] $h(u_1, u_1) \in \Rstar \backslash \Rstarsq$.
...
In our example, because $h$ was defined in terms of $v_1, \dotsc, v_m$, this basis will be orthogonal by the definition of $h$,
...
Moreover, $\overline{V} = V / V \mathfrak{i}$ is a free $\overline{A}$ module of rank $m$.
Define  $\overline{h}: \overline{V} \times \overline{V} \to \overline{A}$,
given by $\overline{h}(v + V \mathfrak{i}, w + V \mathfrak{i}) = h(v, w)$.
$\overline{h}$ can be verified to be well-defined and non-degenerate.
...
Thus by \Cref{theorem3.5} the discriminant of $h$ is $1$ or $\varep$.
Two hermitian forms $h_1$ and $h_2$ are said to be \textbf{equivalent} if there exists an isomorphism $T: V \to V$ such that
$h_1(v, w) = h_2(Tv, Tw)$ for all $v, w \in V$.
...
Then there exists some isomorphism $T: V \to V$ such that $h_1(v,w) = h_2(Tv, Tw)$ for all $v, w \in V$.
Then
\[
...
given in \Cref{theorem3.5} and that $\overline{h_1}(v + V\frakr, w + V \frakr) = \overline{h_2}(T(v + V \frakr), T(w + V \frakr))$ 
for some invertible $T: \overline{V} \to \overline{V}$ and all $v \in V$.
Then $T (v_1 + V \frakr), \dotsc, T (v_m + V \frakr)$ is a basis of $\overline{V}$; and is orthogonal with respect to $\overline{h_2}$.
Let $\Delta$ represent the discriminant function.
Using the fact that the determinant is invariant under choice of basis:
\begin{align*}
\Delta(\overline{h_1}) &= \prod_{i=1}^m \overline{h}_1(v_i + V \frakr, v_i + V \frakr)\\
&= \prod_{i=1}^m \overline{h_2}(T(v_i + V \frakr), T(v_i + V \frakr))\\ &
= \Delta(\overline{h2})
\end{align*}
...
Then $h_1$ and $h_2$ are equivalent, with $h_2(u_i, u_i) = 1 = h_1(v_i, v_i) = h_1(Tu_i, Tu_i)$ for all $i$.
Similarly, the reductions of $h_1$ and $h_2$ modulo $\frakr$ are equivalent, because each has a Gram matrix of the identity matrix with respect to the basis by which they are defined.
This also implies that the discriminants of $\overline{h_1}$ and $\overline{h_2}$ are equal.
In fact, in our example, because $A$ is commutative, the notion of a determinant is well defined for $h_1$ and $h_2$.
Thus we can see that $h_1$ and $h_2$ have the same discriminant (1 in this example) by taking the determinant of the Gram matrix with for $h_1$ with respect to the basis $v_1, \dotsc, v_m$ 
and for $h_2$ with respect to the basis $u_1, \dotsc, u_m$.
\end{example}
...
The Hermitian form $h$ is of type $\{r_1, \dotsc, r_m \}$ and $\{s_1, \dotsc, s_m \}$ if and only if \newline $(r_1 \dotsb r_m)(s_1 \dotsb s_m)^{-1} \in Q(\Astar)$.
\end{lemma}

\begin{proof}
Assume that $h$ is of type $\{r_1, \dotsc, r_m\}$ and of type $\{s_1, \dotsc s_m \}$. Because the determinant is invariant under the choice of basis, $r_1 \dotsb r_m = s_1 \dotsb s_m$ and $(r_1 \dotsb r_m)(s_1 \dotsb s_m)^{-1} = 1 \in Q(\Astar)$.
...
Recall that $\varep$ is a fixed element of $\Rstar \backslash \Rstarsq$, and let $\delta \in \{1, \varep \}$.
By \Cref{extralemma1}, if $r (-1)^k \delta ^{-1} \in Q(\Astar)$, then $h$ is of type $\{1, -1, \dotsc, 1, -\delta\}$ ($m$ even) or $\{1, -1, \dotsc, 1, -1, -\delta \}$ ($m$ odd).
Note that because $-1 \not\in Q(\Astar) = \Rstarsq$ and $\varep \not \in Q(\Astar)$, then $-\varep \in Q(\Astar)$ (because $[\Rstar : \Rstarsq] = 2$), and $-\varep^{-1} \in Q(\Astar)$ as well.
Consider 4 cases:
\begin{description}
\item[Case 1: $r = 1$ and $k$ even.] Let $\delta = 1$. Then $r (-1)^k \delta^{-1} = 1 \in Q(\Astar)$ and $h$ is of type $\{1, -1, \dotsc,1, -1 \}$.
\item[Case 2: $r = 1$ and $k$ odd.] Let $\delta = \varep$. Then $r (-1)^k \delta^{-1} = -\varep^{-1} \in Q(A)$ and $h$ is of type $\{1, -1, \dotsc, -1, -\varep \}$.
\item[Case 3: $r = \varep$ and $k$ even.] Let $\delta = \varep$. Result follows similarly to case 2,
and $h$ is of type $\{1, -1, \dotsc, 1, -\varep \}$
\item[Case 4: $r = \varep$ and $k$ odd.] Let $\delta = 1$. Result follows similarly to case 1,
and $h$ is of type $\{1, -1, \dotsc, -1, -1\}$.
\end{description}
\end{proof}
Additionally, it is clear from these prior results that $h$ is of kind I and kind II if and only if $Q(\Astar) = \Rstar$.
The following result classifies the range of $h$ based on the dimension of $m$ and the type of $h$.
...
Then $-h(u_3, u_3) \in \Rstar$ and by \Cref{prop3.4}, there exists a primitive vector $v \in u_1 A \oplus u_2 A$ with $h(v,v) = -h(v_3, v_3)$.
Then $u = v + u_3$ is primitive with $h(u,u) = 0$, and applying lemma 3.1 shows that $\Lambda = R$.
\end{proof}

\begin{corollary}\label{cor1}
When $m = 1$, $h$ cannot be isotropic. When $m = 2$, $h$ is isotropic if and only if it is of type $\{1, -1\}$. When $m \ge 3$, $h$ must be isotropic.
\end{corollary}
\begin{proof}
Follows directly from preceding result.
\end{proof}
...
\end{description}
This shows, that by \Cref{cor1}, $h$ from our example is not isotropic if $m = 1$ or $m = 2$.
\end{example}
